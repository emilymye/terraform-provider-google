...
func testSweepComposerEnvironments(config *Config) error {
	...
	for _, e := range found.Environments {
		...
		if err != nil {
			return fmt.Errorf("Environment %q has invalid create time %q", e.Name, e.CreateTime)
		}
		// Skip environments that were created in same day - this is mostly to clean out very old environments.
		if time.Now().Sub(createdAt) < (time.Hour * 24) {
			continue
		}
